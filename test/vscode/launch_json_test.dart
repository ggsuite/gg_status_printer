...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_status_printer.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggStatusPrinter = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_status_printer.dart',
      );

      expect(
        ggStatusPrinter,
        isNotNull,
        reason: 'Item for executing bin/gg_status_printer.dart not found',
      );

      expect(ggStatusPrinter['name'], 'gg_status_printer.dart');
      expect(ggStatusPrinter['type'], 'dart');
      expect(ggStatusPrinter['request'], 'launch');
